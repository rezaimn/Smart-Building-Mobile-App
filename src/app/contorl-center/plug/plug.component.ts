...
export class PlugComponent {
  ...
  getAllHvacDevices() {
    ...
    this
      .httpService
      .get('/rsb-vera/ems/vera/device/getAllVeraOzwDevice?campusId=' + this.selectedCampusId + '&buildingId=' + this.selectedBuildingId + '&floorId=' + this.selectedFloorId + '&workingState=-1&type=-1&queryType=PLUG' + `&size=` + 10000 + `&page=` + 0)
      .subscribe(res => {
  ...
  togglePlug() {
    this.hvacList[this.selectedSPDIndex].isProcessing=true;
    let valueInt = 0;
    if (this.hvacList[this.selectedSPDIndex].deviceworkingstate == '1') {
      valueInt =0;
    } else {
      valueInt =1;
    }
    let data = {
      operationType: 3,
      endPointUniqueId:this.hvacList[this.selectedSPDIndex].deviceid,
      state:valueInt
    };
    this.httpService
      ...
      .subscribe(data => {
        ...
        (error)=>{
          this.hvacList[this.selectedSPDIndex].isProcessing=false;
          this.hvacList[this.selectedSPDIndex].on=!this.hvacList[this.selectedSPDIndex].on;
        }
      );
  }

  openModal(modalId, index) {
    this.selectedSPDIndex = index;
